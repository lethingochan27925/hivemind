...
docker build -f Dockerfile.lambda-go --build-arg SERVICE_PATH=./cmd/worker -t hivemind-worker:latest .

echo "Building hivemind-dispatcher (Go)..."
docker build -f Dockerfile.lambda-go --build-arg SERVICE_PATH=./cmd/dispatcher -t hivemind-dispatcher:latest .

echo "Building hivemind-heartbeat-reaper (Go)..."
docker build -f Dockerfile.lambda-go --build-arg SERVICE_PATH=./cmd/heartbeat-reaper -t hivemind-heartbeat-reaper:latest .

echo "Building hivemind-salience-decay (Go)..."
docker build -f Dockerfile.lambda-go --build-arg SERVICE_PATH=./cmd/salience-decay -t hivemind-salience-decay:latest .
