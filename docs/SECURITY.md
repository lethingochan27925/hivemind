...
- `.gitignore` blocks `*.tfstate`, `*.tfvars`, and every `.env*` (except examples). A pre-push secret scan checks staged content for connection strings, private keys, and AWS keys. The 470 MB PaySim CSV and DB backups are ignored too — data is regenerated by `scripts/init.sh`, not stored in git.
