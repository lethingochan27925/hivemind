...
function Node({
  ...
  return (
    <div className="relative flex flex-col gap-1.5 min-w-[150px] rounded-xl border border-border bg-bg-panel px-4 py-3 shadow-[0_1px_0_0_rgba(255,255,255,0.03)_inset,0_2px_8px_-2px_rgba(0,0,0,0.5)]">
    ...
    <div className="rounded-xl border border-border bg-bg-inset/60 p-5 bg-[radial-gradient(circle_at_1px_1px,rgba(255,255,255,0.04)_1px,transparent_0)] bg-[length:22px_22px]">
      <div className="space-y-1">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              <Node icon={<Brain size={16} />} title="Claude Haiku" sub="reasoning · verdicts" accent="orange" />
              <Node icon={<Brain size={16} />} title="Titan Embeddings" sub="1024-dim vectors" accent="teal" />
            </div>
          </div>
        </div>
        ...
        <Tier label="Platform services">
          <Node icon={<CalendarClock size={16} />} title="EventBridge" sub={`${count("events")} schedules`} accent="yellow" />
          <Node icon={<Activity size={16} />} title="CloudWatch" sub={`${count("cloudwatch")} alarms · ${count("logs")} log groups`} accent="green" />
          <Node icon={<KeyRound size={16} />} title="SSM" sub={`${count("ssm")} parameters`} accent="blue" />
          <Node icon={<Package size={16} />} title="ECR" sub={`${count("ecr")} repositories`} accent="purple" />
          <Node icon={<Bell size={16} />} title="SNS" sub={`${count("sns")} topic`} accent="red" />
          <Node icon={<Table2 size={16} />} title="DynamoDB" sub={`${count("dynamodb")} tfstate lock`} accent="teal" />
        </Tier>
      </div>
    </div>
  );
